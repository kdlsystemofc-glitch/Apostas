...
export default function BTTSBlock({ homeName, awayName, pBtts, details, realValue }) {
  ...
  const palpiteObj = { palpite: pBtts >= 0.50 ? "Ambas Marcam: SIM" : "Ambas Marcam: NÃO" };
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-lg bg-slate-950/60 p-3 text-center border border-slate-800">
            ...
            <p className="text-sm font-extrabold text-emerald-400 tabular-nums">{(details.p_casa_marca * 100).toFixed(1)}%</p>
          </div>
          <div className="rounded-lg bg-slate-950/60 p-3 text-center border border-slate-800">
            ...
            <p className="text-xs text-slate-400 mt-1 font-semibold">P(Marca)</p>
            <p className="text-sm font-extrabold text-emerald-400 tabular-nums">{(details.p_fora_marca * 100).toFixed(1)}%</p>
          </div>
        </div>
      </div>
    </div>
